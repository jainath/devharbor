...
| 11 | DevHarbor theme refresh | ✅ | New look: **teal accent `#2dd4bf`** + neutral near-black canvas, **single glowing status dot** (running=green+pulse) consistent across sidebar/dashboard/recent/detail, **thin CPU progress bar** (replaces sparkline), teal port numbers. Dashboard icon → Boxes, **count badge** = 18px green circle with dark text, settings gear → bottom-left dim. "Open in" detection fixed (`.app` bundle existence + `open -a`, PATH-independent). Color picker removed. Detail header **action row regrouped** (uniform icon buttons │ filled primary │ isolated destructive). |
...
- **2026-06-11** - Improvement-plan hardening + feature pass (see [`IMPROVEMENT-PLAN.md`](IMPROVEMENT-PLAN.md)). A comprehensive internal audit surfaced 88 findings; this pass implemented the great majority across six waves. Highlights:
  - **Add-app redesign shipped** - single progressive screen (browse → detect → name/Node/PM/script → optional env), now a **single atomic `apps:create` IPC** that commits app + tasks + env in one transaction (no renderer-side rollback). New welcome **EmptyState** teaches first-run. Monorepo: `apps:detect` reads `pnpm-workspace.yaml` / `workspaces`, and the drawer offers "create a task per workspace package."
  - **Correctness (P1 bugs):** restart-on-change was silently dead (chokidar v5 dropped globs) - now watches the dir with a function-ignore + picomatch filter; previously-run apps were **unremovable** (`!== 'idle'` vs sticky outcome) - guard now blocks only live states; failed starts no longer strand the app in "starting" (terminal state always emitted); `TaskRunner.start` double-spawn + stale-exit races fixed (in-flight dedup + closure-identity guards); readiness timeout; SIGTERM→grace→SIGKILL now **tree-kills**; PTY-orphan-on-history-throw guarded. **Graceful quit** confirms + reverse-topo stops every server (incl. auto-update install).
  - **Data:** migration `0006` rebuilds `env_vars` with scope-aware **partial unique indexes** (task-overrides-app key now saves); deleted task vars no longer resurrect; **secret env values encrypted at rest** via `safeStorage`; `run_history` pruned on boot; corrupt-JSON cells no longer break listing; WAL-safe `db:export`/`db:reset` + migration-failure dialog.
  - **Security/Electron:** Electron **fuses** (disable RunAsNode/NODE_OPTIONS/--inspect, OnlyLoadAppFromAsar) via afterPack; trimmed `disable-executable-page-protection`; tightened CSP; project `.env` can no longer override user-set env or PATH/process-control keys; `apps:update` validates + pins identity fields; deep-link paths require a user gesture before any FS read.
  - **Perf:** StatsMonitor batches `pidusage` (1 fork/tick), PortDetector snapshots the process table once (1 `ps` + 1 `lsof`/tick) + prunes stale hints, LogBuffer has a 100 MB global cap with exited-buffer eviction, visibility-gated log streaming, `tasks:listAll` removes the boot N+1.
  - **Accessibility:** focusable sidebar rows, `useDialog` (focus trap / Escape / restore) on every drawer, keyboard-navigable ContextMenu + dropdowns, StatusDot `role=img` + non-color crash cue, icon-button `aria-label`s, screen-reader xterm + light terminal theme.
  - **Observability/infra:** local file logger (`~/Library/Logs/DevHarbor`) + uncaughtException/unhandledRejection + renderer `render-process-gone` auto-reload; ErrorBoundary; window bounds persisted; **CI workflow** (typecheck + lint + test on PR/push), **ESLint flat config + Prettier**, release workflow bumped to Node-24-native action majors + tag-match publish guard.
  - **New features:** menubar **Tray** (aggregate state + per-app start/stop + Stop all), **crash/ready notifications**, **launch-at-login + per-app auto-start** (`0007`), **start/stop all in folder**, **bulk import** (scan a folder of repos), **global log search** (⌘⇧F), **port-conflict** callout in CrashPin, update **release notes** + error surfacing in the banner.
  - Verification: typecheck + lint (83 files, 0 warnings) + 52 tests (incl. new restart-watcher and double-start-race coverage) + production build all green; all 7 migrations validated against a real SQLite DB.
